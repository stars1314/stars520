...
class Category(Resource):
    def get(self):
        try:
            ...
            if c:
                return to_get_status(200, c.to_dict(), '获取商品分类成功！！')
            else:
    ...
    def post(self):
        try:
            ...
            if all([name, level]):
                ...
                db.session.add(c)
                db.session.commit()
                return to_get_status(200, msg='增加商品分类成功！！')
    ...
    def put(self):
        try:
            ...
            if c:
                c.name = name
                db.session.commit()
                return to_get_status(200, msg='更新商品分类成功！！')
            return to_get_status(1012)
        except Exception as e:
            return to_get_status(2000)
    ...
    def delete(self):
        ...
        if c:
            db.session.delete(c)
            db.session.commit()
            return to_get_status(200, msg='删除商品分类成功！')
        return to_get_status(1012)
...
def get_categroy_list():
    ...
    if all([pnum, psize]):
        ...
        return to_get_status(200, data, '获取商品分类列表成功！！')
    else:
        ...
        return to_get_status(200, {'data': cate_list}, '获取商品分类列表成功！！')
...
class Attribute(Resource):
    def post(self):
        try:
            ...
            if all([name, cid, _type]):
                ...
                db.session.add(attr)
                db.session.commit()
                return to_get_status(200, msg="增加分类参数成功！！")
            else:
                return to_get_status(1000)
        except Exception as e:
            print(e)
    ...
    def get(self):
        ...
        if attr:
            return to_get_status(200, attr.to_dict(), msg="获取分类参数成功！！")
        else:
            return to_get_status(1012)
        ...
        try:
            ...
            if all([id, cid, name]):
                ...
                if attr:
                    ...
                    attr.cid = cid
                    db.session.commit()
                    return to_get_status(200, msg='更新数据成功！！')
                else:
                    return to_get_status(1012)
            else:
                return to_get_status(1000)
    ...
    def delete(self):
        ...
        if attr:
            ...
            db.session.commit()
            return to_get_status(200, attr.to_dict(), msg='删除分类参数成功！！')
        else:
            return to_get_status(1012)
...
def get_attr_list():
    ...
    if all([cid, _type]):
        ...
        if cate:
            ...
            return to_get_status(200, attr_list, msg="获取分类属性列表成功！！")
        else:
            return to_get_status(1012)
    else:
        return to_get_status(1000)
...
def get_cate_group_by_level():
    ...
    return to_get_status(200,data,'获取统计数据成功！！')