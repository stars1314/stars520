...
def get_goods_list():
    ...
    return to_get_status(200, goods_list, '获取商品列表成功！！')
...
class Goods(Resource):
    def post(self):
        try:
            ...
            db.session.commit()
            return to_get_status(200, msg="增加商品成功！！")
    ...
    def delete(self):
        ...
        if goods:
            ...
            db.session.commit()
            return to_get_status(200, msg='删除商品成功！！')
        else:
...
def upload_img():
    ...
    if allowed_img(img_file.filename):
        ...
        return to_get_status(200, data, '上传图片成功！！')
    else:
        return to_get_status(1014)
